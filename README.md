...
sees(graph, state, "camera")                          # {SeesRecord(class='sky.science', terminal='sky', via=(...))}
...
or `selector.aspect` for a multi-axis device (`covercalibrator.calibrator`). A selector without
usable telemetry is `undefined` and validates nothing.
